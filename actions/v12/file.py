...
async def get_file_fragmented(
    stage: str, file_id: str, offset: int | None = None, size: int | None = None
) -> dict:
...
async def upload_file_fragmented(
    ...
    match stage:
        ...
        case "prepare":
            ...
            return return_object.get(file_id=file_id)
...
async def get_file_name_by_id(file_id: str) -> str | None:
    ...
    if cache_data := cached_url_list.get(file_id):
        return await get_file_name_by_id(
            (await upload_file("url", cache_data["name"], cache_data["url"]))["data"][
                "file_id"
            ]
        )
    ...
    for file_id in list(file_list.keys()):
        ...
        if file_id in cached_url_list:
            if config["system"].get("cache_first"):
                logger.warning(
                    f"文件 {file_list[file_id]} ({file_id}) 已被缓存，正在删除储存文件"
                )
                file_list.pop(file_id)
                continue
            else:
                logger.warning(
                    f"文件 {file_list[file_id]} ({file_id}) 已被储存，正在删除缓存索引"
                )
