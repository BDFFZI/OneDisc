...
async def send_group_msg(
    group_id: int, message: str | list[dict], auto_escape: bool = False
) -> dict:
...
async def send_private_msg(
    user_id: int, message: str | list[dict], auto_escape: bool = False
) -> dict:
    if isinstance(message, str) and not auto_escape:
        message = parser.parse_string_to_array(message)
    elif isinstance(message, str):
...
@register_action("v11")
async def get_stranger_info(user_id: int, no_cache: bool = False) -> dict:
    ...
    return translator.translate_action_response(await basic.get_user_info(str(user_id)))


@register_action("v11")
async def get_login_info() -> dict:
    return translator.translate_action_response(await basic.get_self_info())
...
async def set_group_kick(
    ...
    if isinstance(group, PrivateChannel | ForumChannel | CategoryChannel):
        return return_object.get(
            1400, f"不支持的操作：从 {type(group)} 中移除 {user_id}"
        )
...
async def set_group_ban(
    group_id: int, user_id: int, duration: int = 1800, reason: str | None = None
) -> dict:
    if not (group := client.get_channel(group_id)):
        return return_object.get(1400, f"不存在的频道：{group_id}")
    if isinstance(group, PrivateChannel | ForumChannel | CategoryChannel):
        return return_object.get(
            1400, f"不支持的操作：从 {type(group)} 中禁言 {user_id}"
        )
    for user in group.members:
        if user.id == user_id:
        ...
        channel_list.append(
            {
                ...
                "max_member_count": config["system"].get(
                    "default_max_member_count", -1
                ),
            }
        )
...
async def get_status() -> dict:
    return return_object.get(
        0, online=client.is_ready() and not client.is_closed(), good=True
    )
...
async def get_version_info() -> dict:
    return return_object.get(
        0, app_name="onedisc", app_version=version.VERSION, protocol_version="v11"
    )
...
async def set_group_card(group_id: int, user_id: int, card: str) -> dict:
    try:
        client.get_channel(group_id).guild.get_member(user_id).edit(nick=card)
...
async def set_restart() -> dict:
    ...
    return {"status": "async", "retcode": 1, "data": None, "message": ""}
...
async def set_group_admin(group_id: int, user_id: int, enable: bool = True) -> dict:
    ...
    overwrites[member] = discord.PermissionOverwrite(
        manage_channels=enable, manage_permissions=enable
    )
